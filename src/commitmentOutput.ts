...
export const createCommitmentOutput = (flagAssetId: string, pubkey: string) => {
  const flagAssetIdLe = hexLE(flagAssetId);

  //const caseStaticValue = isAddLiquidity ? "51" : "00";
  //No longer used

  const commitmentOutput = "00c86920" + flagAssetIdLe + "876351675ab276a914" + crypto.hash160v2(WizData.fromHex(pubkey)) + "88ac68";

  return commitmentOutput;
};

export const commitmentOutputTapscript = (flagAssetId: string, pubkey: string) => {
  const commitmentOutput = createCommitmentOutput(flagAssetId, pubkey);
